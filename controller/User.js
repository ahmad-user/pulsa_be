...
// UPLOAD IMAGES
 ...
 const Register = async(req, res) => {
    ...
    if (!validator.isEmail(email)) return res.status(400).json({msg: "Email tidak sesuai"});
    if (password.length < 8) return res.status(400).json({msg:"password harus 8 kharakter"});
    if (password != confirm_password) return res.status(400).json({msg: "password tidak sesuai"});
