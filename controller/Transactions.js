...
const Users = require('../models/UserModels.js');
    ...
    // POST TRANSACTION
     const TambahTransaction = async (req, res) => {
        ...
        try {
            const service = await Service.findOne({ where: { service_code: service_code } });
    ...
    // TOP UP AMMOUNT
     const TopUpAmount = async (req, res) => {
        ...
        if (isNaN(top_up_amount) || top_up_amount < 5000) {
            return res.status(400).json({
                status: 102,
                message: "Parameter amount hanya boleh angka dan tidak boleh lebih kecil dari 5000",
                data: null
            });
        }
        try {
            ...
            user.top_up_amount = (user.top_up_amount || 0) + parseInt(top_up_amount, 10);
            await user.save();
    ...
    // GET HISTORI TRANSACTION
     const GetHistory = async (req, res) => {
        try {
            ...
            if (isNaN(offsetNumber) || isNaN(limitNumber) || offsetNumber < 0 || limitNumber <= 0) {
                return res.status(400).json({
                    status: 1,
            ...
            const history = await Transaction.findAndCountAll({
                attributes: ['invoice_number', 'service_code', 'service_name', 'transaction_type', 'total_amount', 'createdAt'],
                offset: offsetNumber,
                limit: limitNumber
            });
